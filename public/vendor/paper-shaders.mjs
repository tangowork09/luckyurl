/* esm.sh - @paper-design/shaders@0.0.77 */
...
}`;function Ee(o){return"paperShaderMount"in o}function Oe(){let o=navigator.userAgent.toLowerCase();return o.includes("safari")&&!o.includes("chrome")&&!o.includes("android")}function De(o){let e=visualViewport?.scale??1,a=visualViewport?.width??window.innerWidth,r=window.innerWidth-o.documentElement.clientWidth,u=e*a+r,n=outerWidth/u,c=Math.round(100*n);return c%5===0?c/100:c===33?1/3:c===67?2/3:c===133?4/3:n}var Pe={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Ge={fit:"none",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Te={none:0,contain:1,cover:2};var d=`
...
`;var pe={maxColorCount:10},Ne=`#version 300 es
precision mediump float;
...
`;var se={maxColorCount:10,maxNoiseIterations:8},Qe=`#version 300 es
precision mediump float;
...
`;var de={maxColorCount:10},Ye=`#version 300 es
precision mediump float;
...
`,He={circle:0,diamond:1,square:2,triangle:3};var ge={maxColorCount:10},Ke=`#version 300 es
precision mediump float;
...
void main() {
  ...
  vec4 gradient = u_colors[0];
  gradient.rgb *= gradient.a;
  for (int i = 1; i < ${ge.maxColorCount}; i++) {
    if (i >= int(u_colorsCount)) break;
...
`;var le={maxColorCount:8,maxBallsCount:20},Xe=`#version 300 es
precision mediump float;
...
void main() {
  ...
  for (int i = 0; i < ${le.maxBallsCount}; i++) {
    if (i >= int(ceil(u_count))) break;

    float idxFract = float(i) / float(${le.maxBallsCount});
...
`;var ve={maxColorCount:5},Je=`#version 300 es
precision mediump float;
...
`;var he={maxColorCount:10},Ze=`#version 300 es
precision mediump float;
...
`,$e={checks:0,stripes:1,edge:2};var xe={maxColorCount:5},eo=`#version 300 es
precision mediump float;
...
`;var _e={maxColorCount:10},to=`#version 300 es
precision mediump float;
...
`,io={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},ro={random:1,"2x2":2,"4x4":3,"8x8":4};var ye={maxColorCount:7},so=`#version 300 es
...
`,lo={wave:1,dots:2,truchet:3,corners:4,ripple:5,blob:6,sphere:7};var ne={maxColorCount:5,maxSpots:4},no=`#version 300 es
precision lowp float;
...
void main() {
  ...
  fragColor = vec4(color, opacity);
}`,co={auto:0,square:1};var ce={maxColorCount:7},uo=`#version 300 es
precision lowp float;
...
void main() {
  ...
  vec4 premultipliedColors[${ce.maxColorCount}];
  for (int i = 0; i < ${ce.maxColorCount}; i++) {
...
`;var Ae={maxColorCount:10},fo=`#version 300 es
precision mediump float;
...
`;var be={maxColorCount:10},mo=`#version 300 es
precision mediump float;
...
void main() {
  ...
  float outerShape = 0.;
  for (int i = 1; i < ${be.maxColorCount+1}; i++) {
    if (i > int(u_colorsCount)) break;
...
`,ho={lines:1,linesIrregular:2,wave:3,zigzag:4,pattern:5},xo={prism:1,lens:2,contour:3,cascade:4,flat:5};var _o=`#version 300 es
precision mediump float;
...
`;var we={maxColorCount:10},yo=`#version 300 es
...
void main() {
  ...
  gradient.rgb *= gradient.a;
  float outerShape = 0.;
  for (int i = 1; i < ${we.maxColorCount+1}; i++) {
    if (i > int(u_colorsCount)) break;
    float m = clamp(mixer - float(i - 1), 0., 1.);
    if (i == 1) {
      outerShape = m;
...
`;function Ao(o){let e=document.createElement("canvas"),a=1e3;return new Promise((r,u)=>{let n=new Image;n.crossOrigin="anonymous",n.addEventListener("load",()=>{(typeof o=="string"?o.endsWith(".svg"):o.type==="image/svg+xml")&&(n.width=a,n.height=a);let c=n.naturalWidth/n.naturalHeight,t=Math.floor(a*.15),p=Math.ceil(t*2.5),s=a,g=a;c>1?g=Math.floor(a/c):s=Math.floor(a*c),e.width=s+2*p,e.height=g+2*p;let f=e.getContext("2d",{willReadFrequently:!0});if(!f)throw new Error("Failed to get canvas 2d context");f.fillStyle="white",f.fillRect(0,0,e.width,e.height),f.drawImage(n,p,p,s,g);let{width:m,height:I}=e,v=f.getImageData(0,0,m,I).data,i=m*I,l=new Uint8ClampedArray(i);for(let M=0;M<i;M++){let E=M*4,O=v[E]??0,X=v[E+1]??0,L=v[E+2]??0;l[M]=.299*O+.587*X+.114*L|0}let A=t,w=Math.max(1,Math.round(.12*t)),V=5,R=Ce(l,m,I,A,3),C=Ce(l,m,I,w,3),x=Ce(l,m,I,V,1),F=f.createImageData(m,I),Y=F.data;for(let M=0;M<i;M++){let E=M*4;Y[E]=x[M]??0,Y[E+1]=R[M]??0,Y[E+2]=C[M]??0,Y[E+3]=255}f.putImageData(F,0,0),e.toBlob(M=>{if(!M){u(new Error("Failed to create PNG blob"));return}r({blob:M})},"image/png")}),n.addEventListener("error",()=>{u(new Error("Failed to load image"))}),n.src=typeof o=="string"?o:URL.createObjectURL(o)})}function Re(o,e,a,r){if(r<=0)return o.slice();let u=new Uint8ClampedArray(e*a),n=new Uint32Array(e*a);for(let c=0;c<a;c++){let t=0;for(let p=0;p<e;p++){let s=c*e+p,g=o[s]??0;t+=g,n[s]=t+(c>0?n[s-e]??0:0)}}for(let c=0;c<a;c++){let t=Math.max(0,c-r),p=Math.min(a-1,c+r);for(let s=0;s<e;s++){let g=Math.max(0,s-r),f=Math.min(e-1,s+r),m=p*e+f,I=p*e+(g-1),D=(t-1)*e+f,v=(t-1)*e+(g-1),i=n[m]??0,l=g>0?n[I]??0:0,A=t>0?n[D]??0:0,w=g>0&&t>0?n[v]??0:0,V=i-l-A+w,R=(f-g+1)*(p-t+1);u[c*e+s]=Math.round(V/R)}}return u}function Ce(o,e,a,r,u){if(r<=0||u<=1)return Re(o,e,a,r);let n=o,c=o;for(let t=0;t<u;t++)c=Re(n,e,a,r),n=c;return c}var bo=`#version 300 es
precision mediump float;
...
`,q={measurePerformance:!1,workingSize:512,iterations:40};function Co(o){let e=document.createElement("canvas"),a=e.getContext("2d"),r=typeof o=="string"&&o.startsWith("blob:");return new Promise((u,n)=>{if(!o||!a){n(new Error("Invalid file or canvas context"));return}let c=r&&fetch(o).then(s=>s.headers.get("Content-Type")),t=new Image;t.crossOrigin="anonymous";let p=performance.now();t.onload=async()=>{let s,g=await c;g?s=g==="image/svg+xml":typeof o=="string"?s=o.endsWith(".svg")||o.startsWith("data:image/svg+xml"):s=o.type==="image/svg+xml";let f=t.width||t.naturalWidth,m=t.height||t.naturalHeight;if(s){let _=f/m;f>m?(f=4096,m=4096/_):(m=4096,f=4096*_),t.width=f,t.height=m}let I=Math.min(f,m),v=q.workingSize/I,i=Math.round(f*v),l=Math.round(m*v);q.measurePerformance&&(console.log("[Processing Mode]"),console.log(`  Original: ${f}\xD7${m}`),console.log(`  Working: ${i}\xD7${l} (${(v*100).toFixed(1)}% scale)`),v<1&&console.log(`  Speedup: ~${Math.round(1/(v*v))}\xD7`)),e.width=f,e.height=m;let A=document.createElement("canvas");A.width=i,A.height=l;let w=A.getContext("2d");w.drawImage(t,0,0,i,l);let V=performance.now(),C=w.getImageData(0,0,i,l).data,x=new Uint8Array(i*l),F=new Uint8Array(i*l),Y=0;for(let h=0,_=0;h<C.length;h+=4,_++){let z=C[h+3]===0?0:1;x[_]=z,Y+=z}let M=[],E=[];for(let h=0;h<l;h++)for(let _=0;_<i;_++){let S=h*i+_;if(!x[S])continue;let z=!1;_===0||_===i-1||h===0||h===l-1?z=!0:z=!x[S-1]||!x[S+1]||!x[S-i]||!x[S+i]||!x[S-i-1]||!x[S-i+1]||!x[S+i-1]||!x[S+i+1],z?(F[S]=1,M.push(S)):E.push(S)}q.measurePerformance&&(console.log(`[Mask Building] Time: ${(performance.now()-V).toFixed(2)}ms`),console.log(`  Shape pixels: ${Y} / ${i*l} (${(Y/(i*l)*100).toFixed(1)}%)`),console.log(`  Interior pixels: ${E.length}`),console.log(`  Boundary pixels: ${M.length}`));let O=wo(x,F,new Uint32Array(E),new Uint32Array(M),i,l),X=performance.now(),L=So(O,x,F,i,l);q.measurePerformance&&console.log(`[Poisson Solve] Time: ${(performance.now()-X).toFixed(2)}ms`);let j=0,H;for(let h=0;h<E.length;h++){let _=E[h];L[_]>j&&(j=L[_])}let $=document.createElement("canvas");$.width=i,$.height=l;let ae=$.getContext("2d"),Q=ae.createImageData(i,l);for(let h=0;h<l;h++)for(let _=0;_<i;_++){let S=h*i+_,z=S*4;if(!x[S])Q.data[z]=255,Q.data[z+1]=255,Q.data[z+2]=255,Q.data[z+3]=0;else{let oe=255*(1-L[S]/j);Q.data[z]=oe,Q.data[z+1]=oe,Q.data[z+2]=oe,Q.data[z+3]=255}}ae.putImageData(Q,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage($,0,0,i,l,0,0,f,m);let T=a.getImageData(0,0,f,m),ee=document.createElement("canvas");ee.width=f,ee.height=m;let J=ee.getContext("2d");J.drawImage(t,0,0,f,m);let ie=J.getImageData(0,0,f,m);for(let h=0;h<T.data.length;h+=4){let _=ie.data[h+3],S=T.data[h+3];_===0?(T.data[h]=255,T.data[h+1]=0):(T.data[h]=S===0?0:T.data[h],T.data[h+1]=_),T.data[h+2]=255,T.data[h+3]=255}a.putImageData(T,0,0),H=T,e.toBlob(h=>{if(!h){n(new Error("Failed to create PNG blob"));return}if(q.measurePerformance){let _=performance.now()-p;if(console.log(`[Total Processing Time] ${_.toFixed(2)}ms`),v<1){let S=_*Math.pow(f*m/(i*l),1.5);console.log(`[Estimated time at full resolution] ~${S.toFixed(0)}ms`),console.log(`[Time saved] ~${(S-_).toFixed(0)}ms (${Math.round(S/_)}\xD7 faster)`)}}u({imageData:H,pngBlob:h})},"image/png")},t.onerror=()=>n(new Error("Failed to load image")),t.src=typeof o=="string"?o:URL.createObjectURL(o)})}function wo(o,e,a,r,u,n){let c=a.length,t=new Int32Array(c*4);for(let p=0;p<c;p++){let s=a[p],g=s%u,f=Math.floor(s/u);t[p*4+0]=g<u-1&&o[s+1]?s+1:-1,t[p*4+1]=g>0&&o[s-1]?s-1:-1,t[p*4+2]=f>0&&o[s-u]?s-u:-1,t[p*4+3]=f<n-1&&o[s+u]?s+u:-1}return{interiorPixels:a,boundaryPixels:r,pixelCount:c,neighborIndices:t}}function So(o,e,a,r,u){let n=q.iterations,c=.01,t=new Float32Array(r*u),{interiorPixels:p,neighborIndices:s,pixelCount:g}=o,f=performance.now(),m=1.9,I=[],D=[];for(let v=0;v<g;v++){let i=p[v],l=i%r,A=Math.floor(i/r);(l+A)%2===0?I.push(v):D.push(v)}for(let v=0;v<n;v++){for(let i of I){let l=p[i],A=s[i*4+0],w=s[i*4+1],V=s[i*4+2],R=s[i*4+3],C=0;A>=0&&(C+=t[A]),w>=0&&(C+=t[w]),V>=0&&(C+=t[V]),R>=0&&(C+=t[R]);let x=(c+C)/4;t[l]=m*x+(1-m)*t[l]}for(let i of D){let l=p[i],A=s[i*4+0],w=s[i*4+1],V=s[i*4+2],R=s[i*4+3],C=0;A>=0&&(C+=t[A]),w>=0&&(C+=t[w]),V>=0&&(C+=t[V]),R>=0&&(C+=t[R]);let x=(c+C)/4;t[l]=m*x+(1-m)*t[l]}}if(q.measurePerformance){let v=performance.now()-f;console.log(`[Optimized Poisson Solver (SOR \u03C9=${m})]`),console.log(`  Working size: ${r}\xD7${u}`),console.log(`  Iterations: ${n}`),console.log(`  Time: ${v.toFixed(2)}ms`),console.log(`  Interior pixels processed: ${g}`),console.log(`  Speed: ${(n*g/(v*1e3)).toFixed(2)} Mpixels/sec`)}return t}var Bo={none:0,circle:1,daisy:2,diamond:3,metaballs:4};var Uo=`#version 300 es
precision mediump float;
...
`,Vo={classic:0,gooey:1,holes:2,soft:3},ko={square:0,hex:1};var Io=`#version 300 es
precision mediump float;
...
`,Ro={dots:0,ink:1,sharp:2};var Se={maxColorCount:6},Fo=`#version 300 es
precision mediump float;
...
// Smoke UV setup
  ...
  float smokeMask = 0.;
  for (int i = 1; i < ${Se.maxColorCount+1}; i++) {
    if (i > int(u_colorsCount)) break;
